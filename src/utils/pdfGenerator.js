...
const generateDetailedExportPDF = (dashboardData, destinationChartData, salesByDestinationChartData, selectedDestination, selectedVerger, filters) => {
  ...
  // Header
  doc.setFontSize(18);
  doc.text('Export by Verger (Grouped by Variety)', 20, 20);

  doc.setFontSize(10);
  doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 20, 30);
  doc.text(`Period: ${filters.startDate} to ${filters.endDate}`, 20, 37);
  ...
  if (destinationChartData?.data && destinationChartData.data.length > 0) {
    ...
    destinationChartData.data.forEach(item => {
      ...
      allVarieties.forEach(variety => {
        ...
        varietyData[variety][verger] = value;
        varietyTotals[variety] += value;
        grandTotal += value;
      });
    });
    ...
    // Add main header
    const headerRow = ['Variety', ...allVergers.map(v => String(v).toUpperCase()), 'TotalVariety'];
    ...
    allVarieties.forEach(variety => {
      ...
      allVergers.forEach(verger => {
        ...
        row.push(formatNumberWithSpaces(value));
        varietyTotal += value;
      });

      row.push(formatNumberWithSpaces(varietyTotal));
      tableData.push(row);
    });

    // Add total row
    const totalRow = ['TOTAL', ...allVergers.map(v => ''), formatNumberWithSpaces(grandTotal)];
  ...
  doc.setFontSize(8);
  doc.text('Generated by Fruta Client Dashboard', 20, pageHeight - 20);

  // Save the PDF
  const fileName = `export-by-verger-grouped-by-variety-${selectedDestination?.label || 'all'}-${new Date().toISOString().split('T')[0]}.pdf`;

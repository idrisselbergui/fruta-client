...
// New corrected function for aggregated recap data
const generateGlobalVenteEcartRecapPDF = (aggregatedData, filters) => {
  console.log('Starting Global Vente Ecart PDF generation (Recap)', { aggregatedData });
  ...
  doc.setFontSize(20);
  doc.setTextColor(44, 62, 80);
  doc.text('Récapitulatif Ventes Écarts', 20, 20);
  ...
  const tableData = aggregatedData.map(row => {
    totalPoids += row.poidsTotal || 0;
    totalMontant += row.montantTotal || 0;
  ...
  // Save
  const fileName = `Recap_Ventes_Ecarts_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);
};
